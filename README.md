...
- **网关多密钥**：每个客户端 / 人一把 `sk-jai-*`（「网关」页可新建、显示全文、单独吊销），泄露一把只需吊销那一把，不必把所有客户端一起换掉；另保留「轮换全部密钥」用于一键换掉所有（旧客户端会立即失效）。
- **密钥白/黑名单**：每把密钥可单独限制能用哪些渠道 / 模型（「网关」页密钥行 →「规则」）。语义三条：`拒绝`优先、有 `允许`项即白名单、全「默认」即不限制（与未配置时完全一致）。被挡住时返回 **403 `model_not_allowed`** 而不是 404，`/v1/models` 也按同一把密钥过滤；弹窗里带「当前密钥可访问的模型」实时预览。
