...
**状态**：M0–M9 全部里程碑完成 · UI 2.0（阶段 0–6）已落地 · MCP 统一代理（`server__tool` 转发 + `skill__` 投递）· WebDAV 多设备同步加固（远端备份管理 / 冲突 diff）· 出站代理配置化 · UX 体验增强（日志复现 / 命令面板 / 健康横幅）· 多模态（图片跨族转换 + 模型输入/输出模态集合）· v0.2.0 · MCP 代理转发语义收紧（结果透传 / 失败冒泡 / 独立等待预算）
...
│ WebDAV 自动同步（变更防抖 + 推/拉各自定时）· 供应商健康检查         │
└─────────────────────────────────┬────────────────────────────────┘
                                  │ 进程内调用
┌─────────────────────────────────▼────────────────────────────────┐
