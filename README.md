...
- **当前版本**：v0.4.0（2026-09-23，见 [CHANGELOG.md](CHANGELOG.md)）
...
- **网关多密钥**：每个客户端 / 人一把 `sk-jai-*`（「网关」页的「密钥管理」可新建、点前缀看全文、单独吊销），泄露一把只需吊销那一把，不必把所有客户端一起换掉；「轮换全部密钥」放在独立的危险区用于一键换掉所有（旧客户端会立即失效）。
- **密钥白/黑名单**：每把密钥可单独限制能用哪些渠道 / 模型（「网关」页密钥行上的状态 chip：未配显示「不限」，配过显示「已限制」，点开即配）。语义三条：`拒绝`优先、有 `允许`项即白名单、全「默认」即不限制（与未配置时完全一致）。被挡住时返回 **403 `model_not_allowed`** 而不是 404，`/v1/models` 也按同一把密钥过滤；弹窗里带「当前密钥可访问的模型」实时预览。
