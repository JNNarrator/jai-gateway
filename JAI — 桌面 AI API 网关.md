...
#### MCP 和 SKILL 管理 ✅ 基础能力已交付，高级编排仍属远期

- **配置托管（低成本）** ✅ 已实现：为 Claude Code、Continue 等客户端导出标准 `mcpServers` JSON（参考 CC-Switch 思路）；Skill 可导出 Markdown 技能包，供 Agent 加载后自行调度
- **MCP 工具自动合并与执行循环** ✅ 已实现（超出原「配置托管」边界）：网关把启用 MCP 工具注入请求工具定义；上游发起 MCP 工具调用时网关自动执行并回填结果继续生成；工具列表 30s TTL 缓存
- **MCP Hub / 高级编排（高成本，远期评估）**：完整能力协商、跨 Server 工具命名冲突消解、调用级鉴权等仍未排期，需独立立项论证
