...
> ✅ **已实施（2026-09-22）**。落地与本文的差异：
>
> 1. **`gw_keys_active` 的排序补了 `rowid DESC`**：`created_at` 只到毫秒，同一毫秒内建的多把密钥靠它排序是**不确定**的（测试实测：三把同毫秒创建时顺序随机）→ UI 列表会「跳」。`rowid` 随插入单调递增，是稳定的次级键。
> 2. **label 归一放在 store 层**（`trim` + 空白串 → NULL）而不是 IPC 层：单一来源，`gw_key_rotate` 与导入路径都自动受益（否则列表里会出现看不见的「空标签」）。
> 3. **`GatewayKeyInfo` 增加 `id` / `revokedAt`**：多密钥下「吊销 / 指定显示全文」都要按 id 定位；`gateway_key_reveal` 因此接受可选 `id`（省略时取最新一把，保持单密钥语义的兼容路径）。
> 4. **`gw_key_active` 保留**（不删）：导出、WebDAV 导入的活跃密钥比对、首次启动自举都在用它，语义是「最新一把」；实现改为 `gw_keys_active` 的第一个。
> 5. **WebDAV 导入语义收窄**：本地只有 ≤1 把时保持原轮换语义（既有测试 `import_rotates_gateway_key_only_on_diff` 守着的正是它）；本地**已有多把**时**只补不删** —— 本机配多把是刻意的（不同客户端 / 不同人），远端快照没资格把它们清掉。**已知局限**：导出仍只带最新一把密钥（快照格式 `jai-export/v1` 未扩展），所以多密钥集合不完整往返；跨机同步的实际用途是「同一个人多台机器共用一把」，这条路径不受影响。
> 6. **鉴权遍历不做短路优化**（按本文的注意点）：不「先比 prefix 再比全文」，全量 `ct_eq`；密钥数量是个位数，成本可忽略。
> 7. **前端**：列表每行 = 前缀 / 备注 / 创建时间 / 最后使用 + 「显示全文」「复制」「吊销」；吊销走 `ConfirmDialog` 二次确认；另加「新建密钥」+ 备注输入 + 「复制最新一把」+ 「轮换全部密钥」（文案明确写清「这是把所有密钥一起换掉」）。「接入信息」类复制动作统一用**最新一把**（原来只有一把，没有歧义；现在必须在文案里说清）。
> 8. **UI 门禁补了一条探针**：`tools/visual-regression/probe-keys.mjs` + `gate.mjs` 判据（全部密钥渲染 / 显示-隐藏全文 / 吊销先二次确认且确认前不动数据 / 新建插到最前且当场显示全文 / 全吊销后空态文案）。判据已实测「改坏就会红」：去掉吊销的二次确认后，探针超时崩溃、门禁直接判红。
>
> 验收：`cargo test --workspace` 全绿（35 套件，新增 `m13_multi_key.rs` 7 用例 + 导入多密钥 1 用例）；`node tools/visual-regression/gate.mjs` 全绿（双尺寸 × 双主题，含新增的多密钥列表判据）。
...
> ✅ **已实施（2026-09-22）**。落地与本文的差异：
>
> 1. **`request_logs.key_id` 砍掉了**（本文标注为「可选，建议做」）。原因不是成本，而是**可达性**：`emit_log` 有 25 个调用点，其中大部分在 `try_candidate` / 转换路径等**看不到请求对象**的辅助函数里。要真填上这个字段，得把 key id 一路穿过好几层函数签名；只在 `dispatch` 层填、别处填 `None`，又会让这一列**半真半假**（比没有更容易误导）。而它原本要回答的「这条为什么被拒」已由 403 的错误文案直接回答（文案里点名了是密钥规则）。真要按密钥做用量归因时，再一次性把上下文穿透做干净。
> 2. **规则项不可能「既允许又拒绝」**：两张表的主键都是 `(key_id, 条目)`，同一项写两次会直接撞 UNIQUE 约束。所以「deny 优先于 allow」落在**写库前把冲突项从 allow 侧摘掉**（`key_rules_set`），而不是「保存时失败」。前端因此用**三态**（允许 / 拒绝 / 默认）而不是两个独立列表 —— 既允许又拒绝本来就没有意义。`KeyRules::allows_*` 里保留的 deny 优先分支只作为内存态的兜底。
> 3. **`key_provider_rules.provider_id` 有外键 + `ON DELETE CASCADE`**：`provider_delete` 是硬删（`DELETE FROM providers`），没有 CASCADE 时「删渠道」会直接报外键冲突。代价是规则项必须指向真实存在的渠道 —— 这正是真实调用路径的形态（候选清单来自库里），且 UI 不会提交不存在的 id。`key_id` / `model_name` 刻意不设外键（前者只有软删，后者对应的不是主键），与 `request_logs.provider_id` 同一取舍。
> 4. **候选清单只列启用中的渠道 × 模型**（`store::keyrules::rule_options`），并且弹窗会把「规则里出现过但当前不在清单里」的条目也带出来显示 —— 否则一保存就会把看不见的旧规则静默抹掉。
> 5. **模型规则按 `model_name` 匹配**（与 SQL 一致）：同名模型挂在多个渠道上时一次覆盖全部渠道。UI 上写明了这一点，避免用户以为要逐个渠道配。
> 6. **规则缓存与桌面端共享同一份**（`GatewayCtx::with_rules` + `AppCore.key_rules`）：`gateway_key_rules_set` 保存后立刻 `invalidate`。否则用户刚写完就去测，要等 5s TTL 才生效，会以为没存上。查库失败按「不限制」处理并打日志（路由本身也要查库，DB 坏了下游照样 500；把规则查询失败升级成全拒只会让所有客户端一起 403）。
> 7. **新增 IPC 三个**：`gateway_key_rules_get` / `gateway_key_rules_set` / `gateway_key_rules_options`；前端入口在**密钥行**上的「规则」按钮（规则是按密钥配的，不该另开一个先挑密钥再配规则的页面）。
> 8. **UI 门禁补一条探针** `tools/visual-regression/probe-rules.mjs`：候选清单渲染 / 回填已保存规则 / 预览随草稿实时变（2→4→2→0→1）/ 无可用模型时的 403 后果提示 / 提交的四个数组与三态**逐字**一致 / 保存后关闭并可读回 / 「已限制」徽标只出现在配过规则的行上。判据已逐条实测「改坏就会红」（改坏 payload、让预览忽略模型规则、去掉徽标与 403 提示，三次都分别红了对应的判据）。
>
> 验收：`cargo test --workspace` 全绿（36 套件；新增 `m13_key_rules.rs` 7 用例 + `store::keyrules` 4 单测）；`node tools/visual-regression/gate.mjs` 全绿（双尺寸 × 双主题，含新增的规则弹窗判据；顺带修掉了「拒绝」选中态在浅色主题下 3.87:1 的对比度不达标）。
>
> **已知局限**：规则**不随 WebDAV 同步**（导出仍只带最新一把密钥，见 T6a 的局限）。同一个人多台机器拉取后需要各自配一次规则。
