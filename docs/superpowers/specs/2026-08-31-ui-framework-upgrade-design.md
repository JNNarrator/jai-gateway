...
- 日期：2026-08-31
- 状态：已评审（用户确认方向，追加阶段 6 平台视觉：自绘标题栏 + 毛玻璃）
- 范围：`ui/` 前端（Tauri 2 + React 19 + Vite 6 + Tailwind）；阶段 6 另涉 `src-tauri/` 下的窗口配置（`tauri.conf.json` 平台覆盖与 capabilities JSON），仍不改 Rust 源码与 `api.ts`/`types.ts` 数据层
...
1. **视觉质感升级**：现代、专业的开发者工具观感；明暗双主题；蓝紫主色；自绘标题栏与窗口毛玻璃（阶段 6，低优先级）
...
| 标题栏与窗口特效 | 自绘标题栏 + 毛玻璃（macOS vibrancy / Windows mica·acrylic），**本轮必做、优先级最低**（阶段 6） | 初稿列为非目标；用户评审决定纳入并排最后 |
...
- 标题栏：阶段 0–5 保留系统原生标题栏；阶段 6 切换自绘标题栏，届时 `AppShell` 顶部加 TitleBar 组件并为 macOS 红绿灯预留安全区（方案见阶段 6）
...
| 阶段 | 内容 | 验收要点 |
| --- | --- | --- |
| **0 基建** | Tailwind 3→4（`@tailwindcss/vite` 插件，删除 postcss 链）；shadcn 初始化 + 拷入基础件；双主题变量与 `ThemeToggle`；sonner 替换旧 toast（签名不变）；App.tsx 机械拆分到 `pages/`；侧边栏导航上线；`goTab` 的 CustomEvent 换成导航 Context（外壳重建时一并处理，调用点仅数处） | 双主题可切换且持久化；9 个页面功能与拆分前一致；跨页跳转（同步→网关）可用；`tsc --noEmit` 零错误 |
...
| **3 MCP + 技能** | 列表 + `Dialog` 居中模态表单（桌面场景，不用侧滑）；空状态与批量操作升级 | 导入/导出/测试连接流程不回归 |
| **4 同步 + 日志 + 设置** | WebDAV 表单与预览流程；日志表着色/筛选/导出；设置页 toggle 化 | 同步与日志功能不回归 |
| **5 打磨** | 空状态统一、键盘导航、aria-label 补齐、删除死代码（旧样式类）、（可选）字体栈优化 | 全页走查清单通过 |
| **6 平台视觉** | 自绘标题栏：macOS 平台覆盖配置用 `titleBarStyle: overlay`（保留原生红绿灯、内容上延），Windows/Linux `decorations: false` + 前端自绘最小化/最大化/关闭按钮；拖拽区用 `data-tauri-drag-region`，capabilities 增补窗口操作权限。窗口毛玻璃：`windowEffects`（macOS vibrancy 需 `macOSPrivateApi`；Windows 11 mica/acrylic），主题背景变量改带 alpha，效果不支持时回退实色 | 双平台标题栏拖拽与窗口操作可用；macOS 红绿灯不遮挡内容；毛玻璃在支持平台生效、不支持平台实色回退；双主题各验收一遍 |

阶段顺序依据：先门面（视觉价值最高）→ 再高频交互（表单/表格）→ 后低频页面；阶段 6 平台特效要动窗口层，等新布局稳定后再做，故排最后。
...
- i18n / 英文界面（保持中文）
- `api.ts`/`types.ts` 数据层改动、Rust 源码改动（阶段 6 仅动 `src-tauri/` 下窗口配置与 capabilities JSON）
...
| 毛玻璃平台差异大（Win11 才有 mica；macOS vibrancy 依赖私有 API） | 平台覆盖配置分治：macOS vibrancy、Win11 mica/acrylic；效果不可用时回退实色主题背景，特效作为渐进增强，不阻塞功能验收 |
| 自绘标题栏行为与原生有差异（拖拽、双击最大化等） | 仅用 Tauri 官方 `data-tauri-drag-region` 与窗口 API，逐项对照原生行为验收（拖拽、双击、最小化、最大化、关闭） |
