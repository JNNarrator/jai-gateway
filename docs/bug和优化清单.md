...
- [~] 5. P1 模型表：已确认由 Table 基座横向内滚 + 加窄窗口提示；列降级待评估。原：760 宽窗口溢出 217px，行内输入被折叠线切 9–14px
  - 【2026-09-17 复核】默认窗口 1180×800 下**已无溢出**（表 894 / 容器 1004，余量 110px）；
    ...
    （8 列的日志表在同尺寸下反而富余 50px，说明是模型表列宽本身偏大）。
  - 列构成与降级候选：模型名 1/5、上游模型 ID 1/5、上下文 1/6、最大输出 1/6、模态（入/出）w-28、
    启用 1/12、操作 w-20。除「模态（入/出）」（只读信息列，112px）外其余都承载输入或操作，
    隐藏会削弱功能；而只隐藏它即可覆盖 77px 缺口。
  - **待决策（需产品取舍）**：是否在 <1024px 隐藏「模态（入/出）」列。若隐藏，建议把该信息
    降级为「模型名」单元内的紧凑 badge 或行 `title`，避免信息凭空消失。本次未擅自改动。
...
- [ ] 11. P3 折叠线硬切与顶部无渐隐：卡片被切半（`客户端接入` 398–660、WebDAV 348–1617）、内容滚动时被标题栏切开
  - 【2026-09-17 复核】现状（`fold.mjs --size=1180x800`）：折叠线「切一半」仅剩 2 处，且都是**滚动中间态**
    ...
    已随窗口尺寸与吸顶条整改消失。
  - 剩余诉求只有「顶部无渐隐遮罩」这一条观感项：内容滚过吸顶条/标题栏时是硬边。
  - **未做，待决策**：需要动全局滚动容器（`App.tsx` 的 `<main>`）加顶部渐变遮罩，属纯观感改动，
    且要与 3 处吸顶条（网关/同步/设置）统一处理。本次已解决「主操作可达」这一实质问题（第 4 条），
    故按 P3 缓办，避免在收尾阶段改全局布局。
